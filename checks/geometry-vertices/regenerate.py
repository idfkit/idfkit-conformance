...
THE MIRRORS ARE DROPPED, AND THE COUNT IS RECORDED

EnergyPlus creates a mirrored twin of every detailed shading surface so that it shades from both
sides, names it ``Mir-<original>``, and reports it beside the original. It is the engine's own
object and not the model's, so no extractor should produce it and this file drops it. The number
dropped is written into the expectation's header rather than left silent, so that the exclusion is
auditable from the expectation alone.
...
#: The engine's own prefix for a mirrored shading surface.
MIRROR_PREFIX: Final = "Mir-"
...
def write_expectation(
    ...
    with path.open("w", encoding="utf-8", newline="") as handle:
        ...
        handle.write(f"# mirrored shading surfaces dropped: {mirrors}\n")
...
def regenerate(fixture: Path, energyplus: str) -> str:
    ...
    kept = [surface for surface in every if not surface.name.startswith(MIRROR_PREFIX)]
    write_expectation(name, kept, len(every) - len(kept), engine_version(lines), frame_statement(lines))
    return f"{name}: {len(kept)} surfaces, {len(every) - len(kept)} mirrors dropped"
